...
def display_test_image_info(dictionary):
    ...
    for image in dictionary:
        row_strings += "<tr><td style='text-align: left; border: 1px solid black'>{}</td><td style='font-weight: bold; border: 1px solid black'>{}</td></tr>".format(
            image["path"], image["driver"])
...
def show_image(image, max_size):
    ...
    # 2-channel image — show each channel as grayscale
    elif image.ndim == 3 and image.shape[2] == 2:
        fig, axes = plt.subplots(1, 2, figsize=(
            2 * new_width / 100, new_height / 100))
        for i in range(2):
...
def show_images(images, titles, max_size, columns=4):
    ...
    fig, axes = plt.subplots(rows, columns, figsize=(
        max_size * columns / 100, max_size * rows / 100))
...
def show_scenes(scenes, cols, thumbnail_size):
    ...
    for index in range(scene_count):
        ...
        row_count += 1
        if channel_count > 3:
            image = scene.read_block(size=(thw, 0), channel_indices=[
                                     0, 1, 2], slices=(slice, slice+1))
        elif channel_count == 2:
            image = scene.read_block(size=(thw, 0), channel_indices=[
                                     0], slices=(slice, slice+1))
        else:
...
def create_scene_info_table(scene):
    ...
    for property_name, value in [
        ...
        table += "<tr>"
        table += "<td style='border: 1px solid black; padding: 8px; text-align: left;'>{}</td>".format(
            property_name)
        table += "<td style='border: 1px solid black; padding: 8px; text-align: left;'>{}</td>".format(
            value)
        table += "</tr>"
...
def show_scene_details(scene, size):
    ...
    image_base64 = '<img src="data:image/png;base64,{}">'.format(
...
def extract_image_properties(images):
    ...
    for image in images:
        ...
        for index in range(0, slide.num_scenes):
            ...
            image_info['Compression'] = str(
                scene.compression).replace('Compression.', '')
